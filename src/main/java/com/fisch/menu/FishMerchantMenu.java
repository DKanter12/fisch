...
public class FishMerchantMenu extends AbstractContainerMenu {
     ...
     * ============================================================
     */
    public FishMerchantMenu(int syncId, Inventory playerInventory) {
    ...
    public int getTotalPrice() {
        ...
        for (int i = 0; i < 27; i++) {
            ...
            if (!stack.isEmpty() && ModCommands.FISH_PRICES.containsKey(stack.getItem())) {
                ...
                // Перемножаем на мутацию (если есть)
                int finalPrice = com.fisch.fish.FishMutation.calculatePrice(basePrice, stack);

                total += finalPrice * stack.getCount();
            }
        }
    ...
    public void removed(Player player) {
        ...
        if (!player.level().isClientSide) {
            Containers.dropContents(player.level(), player, this.getMerchantInventory());
        }
    }
}